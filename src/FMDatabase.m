    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (_busyRetryTimeout && (numberOfRetries++ > _busyRetryTimeout)) {
                TMLog(@"%s:%d", __FUNCTION__, __LINE__);
                TMLog(@"Database busy, unable to close");
                return NO;
            }
            ...
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, 0x00)) !=0) {
                    TMLog(@"Closing leaked statement");
                    sqlite3_finalize(pStmt);
                }
            }
        }
        else if (SQLITE_OK != rc) {
            TMLog(@"error closing!: %d", rc);
        }
    }
    while (retry);
...
- (BOOL)rekey:(NSString*)key {
    ...
    if (rc != SQLITE_OK) {
        TMLog(@"error on rekey: %d", rc);
        TMLog(@"%@", [self lastErrorMessage]);
    }
...
- (void)warnInUse {
    TMLog(@"The FMDatabase %@ is currently in use.", self);
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        TMLog(@"The FMDatabase %@ is not open.", self);
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (!pStmt) {
        do {
            ...
            if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
                ...
                if (_busyRetryTimeout && (numberOfRetries++ > _busyRetryTimeout)) {
                    TMLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [self databasePath]);
                    TMLog(@"Database busy");
                    sqlite3_finalize(pStmt);
                    _isExecutingStatement = NO;
                    return nil;
                ...
                if (_logsErrors) {
                    TMLog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                    TMLog(@"DB Query: %@", sql);
                    TMLog(@"DB Path: %@", _databasePath);
#ifndef NS_BLOCK_ASSERTIONS
                    if (_crashOnErrors) {
                        abort();
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (namedIdx > 0) {
                ...
                TMLog(@"Could not find index for %@", dictionaryKey);
            }
        }
    ...
    if (idx != queryCount) {
        TMLog(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return nil;
    }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (idx != queryCount) {
        TMLog(@"Error: the bind count is not correct for the # of variables (%@) (executeUpdate)", sql);
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return NO;
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (SQLITE_LOCKED == rc) {
                ...
                if (rc != SQLITE_LOCKED) {
                    TMLog(@"Unexpected result from sqlite3_reset (%d) eu", rc);
                }
            }
            usleep(20);
            ...
            TMLog(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(_db));
            TMLog(@"DB Query: %@", sql);
        }
        else if (SQLITE_MISUSE == rc) {
            // uh oh.
            TMLog(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(_db));
            TMLog(@"DB Query: %@", sql);
        }
        else {
            // wtf?
            TMLog(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
            TMLog(@"DB Query: %@", sql);
        }
    ...
    if (closeErrorCode != SQLITE_OK) {
        TMLog(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
        TMLog(@"DB Query: %@", sql);
    }
