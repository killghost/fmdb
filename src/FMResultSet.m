...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        TMLog(@"Warning: There seem to be no columns in this set.");
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (SQLITE_LOCKED == rc) {
                ...
                if (rc != SQLITE_LOCKED) {
                    TMLog(@"Unexpected result from sqlite3_reset (%d) rs", rc);
                }
            }
            usleep(20);
            ...
            if ([_parentDB busyRetryTimeout] && (numberOfRetries++ > [_parentDB busyRetryTimeout])) {
                ...
                TMLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
                TMLog(@"Database busy");
                break;
            }
        }
            ...
            TMLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
            break;
        } 
            ...
            // wtf?
            TMLog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
            break;
        }
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    TMLog(@"Warning: I could not find the column named '%@'.", columnName);
